...
*/
    ...
    if(prop->wave_nesting>0){ 
      ...
      for(k=grid->etop[j];k<grid->Nke[j];k++) {
        ...
        if(prop->wave_nesting==1){ //idealized
          ...
          // if( xe>498000. && k==0) printf("sponge z= %f, xe=%f, fz=%f, rtime=%f, utildeprime=%f  \n",z,xe,fz,prop->rtime,u_LS_tilde_prime);
          ...
          // u_LS_tilde_prime = bound->sponge_uf[k][jj];
          if(prop->ramp_on==1) {
          ...
          if(prop->lowfreq_nudging==1){
            u_LS_bar = bound->lowfreq_uf[k][jj];
        ...
        /* This is to allow wave come from one sponge layer but damp wave on the other sponge layer, S.Tan, 05/30/2023, UCI*/
        if(prop->sponge_distance) {  
          // wave flux direction
          if( (prop->Fhat_x*rn1[j]+prop->Fhat_y*rn2[j]) < 0){   // Flux dot rnormal is negative = incoming wave
            F_hat=1;
          ...
          if(prop->ramp_on==1) {
            ...
            //   tr_hat*u_LS_tilde_prime)*D_hat/tauD;
            if (F_hat==0){
              usource[j][k]-= prop->dt * (phys->u[j][k])*D_hat/tauD;
            }else{
              usource[j][k]-= prop->dt * (phys->u[j][k] - 
              tr_hat*u_LS_tilde_prime)*D_hat/tauD;
           }
          }else{
            if (F_hat==0){
  ...
  if(prop->n==prop->nstart+1) {
    //printf("Inlsitializing v Coriolis for a 2.5D simulation\n");
