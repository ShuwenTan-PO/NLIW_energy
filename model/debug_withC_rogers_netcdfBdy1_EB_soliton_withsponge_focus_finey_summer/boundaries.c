...
// if (myproc==0) printf("xmid=%f,ymid=%f\n", prop->xmid,prop->ymid);
  ...
  if(prop->thetaramptime>0){
   ...
   if(prop->netcdfBdy==1){ 
    ...
    // if (prop->rtime<=prop->nbc*prop->dt){
    //     UpdateBdyNC(prop,grid,myproc,comm);
    // }
   }
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    //  phys->boundary_w[jind][k]=(phys->w[ib][k]+phys->w[ib][k+1])/2; //0; //
    ...
    // // nudge u to make sure volume flux is zero
    // for(k=grid->etop[j];k<grid->Nke[j];k++) {
    //   temp1 += grid->dzz[ib][k]*grid->df[j]*phys->boundary_u[jind][k];
    //   temp2 += grid->dzz[ib][k]*grid->df[j];
    // }
    // alpha = -temp1/temp2;
    // for(k=grid->etop[j];k<grid->Nke[j];k++) {
    //   phys->boundary_u[jind][k]+=alpha;
    // }
  }
  }else{ // No NetCDF
...
void InitBoundaryData(propT *prop, gridT *grid, int myproc, MPI_Comm comm){
 ...
 void UpdateBdyNC(propT *prop, gridT *grid, int myproc, MPI_Comm comm){
    ...
    REAL dt, r1, r2, mu, nctime_new, nsoliton_new;

    // nctime lies between time[t0] and time[t1], time[t2] is one step ahead
    if (prop->nsolitons<1){
      // this is to have only one (need to turn on nslitons = 0 in prop)
      nctime_new = -1;
      if (prop->rtime<=prop->nbc*prop->dt){
      // if (prop->rtime<=900){
      ...
      // printf("Updating netcdf boundary data at nc timestep: %d\n",t1);
    }else{
      // this is to have multiple waves (need to turn on nslitons = 0 in prop)
      nsoliton_new = round(prop->dtsoliton*3600.0/prop->dt);
      // printf("Updating netcdf boundary data at nc timestep: %f\n",nsoliton_new);
      if (prop->rtime<=nsoliton_new*prop->dt){
        if (prop->rtime<=prop->nbc*prop->dt){
          nctime_new = prop->nctime;
          t1 = getTimeRecBnd(prop->nctime,bound->time,bound->Nt);
          // printf("Updating netcdf boundary data at nc timestep: %d\n",t1);
        }else{
          t1 = bound->t1;
        ...
        // if (prop->rtime-nsoliton_new*prop->dt<=prop->nbc*prop->dt){
        nctime_new = prop->nctime - floor(prop->rtime/(nsoliton_new*prop->dt))*nsoliton_new*prop->dt;
        // printf("Updating netcdf boundary data at nc timestep: %f\n",nctime_new);
        // if (nctime_new<=prop->nbc*prop->dt){
        if (prop->rtime-floor(prop->rtime/(nsoliton_new*prop->dt))*nsoliton_new*prop->dt<=prop->nbc*prop->dt){
          t1 = getTimeRecBnd(nctime_new,bound->time,bound->Nt);
        }else{
          t1 = bound->t1;
        }
      }
    ...
    if(bound->hasType2>0){
  	...
  	for (j=0;j<bound->Ntype2;j++){
        ...
        if (nctime_new < 0){
          ...
          bound->boundary_u[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->boundary_u_t[0][k][j],bound->boundary_u_t[1][k][j],bound->boundary_u_t[2][k][j] );
          bound->boundary_v[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->boundary_v_t[0][k][j],bound->boundary_v_t[1][k][j],bound->boundary_v_t[2][k][j] );
          bound->boundary_w[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->boundary_w_t[0][k][j],bound->boundary_w_t[1][k][j],bound->boundary_w_t[2][k][j] );
          bound->boundary_T[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->boundary_T_t[0][k][j],bound->boundary_T_t[1][k][j],bound->boundary_T_t[2][k][j] );
          bound->boundary_S[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->boundary_S_t[0][k][j],bound->boundary_S_t[1][k][j],bound->boundary_S_t[2][k][j] );
        }
  	  }
  	}
        ...
        for (j=0;j<grid->Ne;j++){
          for (k=0;k<bound->Nk;k++){
              ...
              if (nctime_new < 0){
                ...
                bound->sponge_uf[k][j] = QuadInterp(nctime_new,bound->time[t0],bound->time[t1],bound->time[t2],bound->sponge_uf_t[0][k][j],bound->sponge_uf_t[1][k][j],bound->sponge_uf_t[2][k][j] );
              }
