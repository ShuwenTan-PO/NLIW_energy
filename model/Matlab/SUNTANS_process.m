...
% dZ_cut = nan+dZ;
% cells below depth
dZ_cut = zeros(size(dZ));
% cut cells vectorized
ZB = 0*dZ;
ZT = 0*dZ;
for i=1:length(z)
    ZB(:,i) = zb(i);
...
dZ_cut(indx)= ZT(indx) + DEPTH(indx);
% create a constant dz, but masked
dZ_mask = dZ;
dZ_mask(dZ_cut == 0) = 0;

% dZ_ = dZ(58326,:);
% dZ_cut_ = dZ_cut(58326,:);
% dZ_mask_ = dZ_mask(58326,:);

clear DEPTH ZB ZT indx
%%
...
if ~isempty(nc_files)
    ...
    if ~isempty(nc_AVG)
        ...
        AVG.Ntout = length(AVG.t_ref);
        AVG.Nt = length(AVG.mtime);
        
        AVG.u(:,:,1:length(step)) = DATA.uc(:,:,step);
        AVG.v(:,:,1:length(step)) = DATA.vc(:,:,step);
        AVG.w(:,:,1:length(step)) = DATA.w(:,:,step);
        AVG.nuT(:,:,1:length(step)) = DATA.nu_v(:,:,step);
        AVG.kappaT(:,:,1:length(step)) = DATA.kappa_tv(:,:,step);
        AVG.S(:,:,1:length(step)) = DATA.salt(:,:,step);
        AVG.T(:,:,1:length(step)) = DATA.temp(:,:,step);
        AVG.eta(1:Nc,1,1:length(step)) = DATA.eta_avg(:,step);
        if isfield(DATA,'q')
            AVG.q = DATA.q(:,:,step);
        end
        if isfield(DATA,'upup')
           AVG.upup(:,:,1:length(step)) = DATA.upup(:,:,step);
           AVG.vpvp(:,:,1:length(step)) = DATA.vpvp(:,:,step);
           AVG.wpwp(:,:,1:length(step)) = DATA.wpwp(:,:,step);
           AVG.upvp(:,:,1:length(step)) = DATA.upvp(:,:,step);
           AVG.upwp(:,:,1:length(step)) = DATA.upwp(:,:,step);
           AVG.vpwp(:,:,1:length(step)) = DATA.vpwp(:,:,step);
        end
...
if exist('AVG','var')
    ...
    if isfield(AVG,'upup')
       AVG.upup(AVG.empties) = nan;
       AVG.vpvp(AVG.empties) = nan;
       AVG.wpwp(AVG.empties) = nan;
       AVG.upvp(AVG.empties) = nan;
       AVG.upwp(AVG.empties) = nan;
       AVG.vpwp(AVG.empties) = nan;
    end
end
...
i=1:Nc;
p0 = zeros(Nc,Nkmax,Nout_keep);
for j=1:Nout_keep
    p0(i,:,j) = rho0*grav*(eta(i,:,j)-Z);
end
if exist('AVG','var')
    AVG.p0 = zeros(Nc,Nkmax,AVG.Nt);
    for j=1:AVG.Nt
        AVG.p0(i,:,j) = rho0*grav*(AVG.eta(i,:,j)-Z);
    end 
end

% background pressure %%%%%%%%%%%%%%%%%%%
% ** NOTE: how dz is defined is really important here. This code defines
% dz as positive and we are integrating in the (-) z direction. 
% assume dZ is constant in (x,y) for this computation.
...
i=1:Nc;
for k = 2:Nkmax
    p_b(i,k,:) = p_b(i,k-1,:)+0.5*rho_b(i,k-1,:).*grav.*dZ_mask(i,k-1)+...
                  0.5*rho_b(i,k,:).*grav.*dZ_mask(i,k);
end
%set values below bottom to nan    
p_b = p_b +0*q;

if exist('AVG','var')
    AVG.p_b = zeros(Nc,Nkmax,AVG.Nt);
    AVG.p_b(:,1,:) = 0.5*AVG.rho_b(:,1,:).*grav.*(dz(1)+AVG.eta); % including the free surface in the pressure
    %    here's a p_b calculation using cut cell dz values at the bottom
    for k = 2:Nkmax
        AVG.p_b(i,k,:) = AVG.p_b(i,k-1,:)+0.5*AVG.rho_b(i,k-1,:).*grav.*dZ_mask(i,k-1)+...
                      0.5*AVG.rho_b(i,k,:).*grav.*dZ_mask(i,k);
    end 
    %set values below bottom to nan    
    AVG.p_b = AVG.p_b +0*AVG.u;
end
...
i=1:Nc;
for k = 2:Nkmax
    p_prime(i,k,:) = p_prime(i,k-1,:)+0.5*rho_prime(i,k-1,:).*grav.*dZ_mask(i,k-1)+...
                       0.5*rho_prime(i,k,:).*grav.*dZ_mask(i,k);
end
p_prime = p_prime +0*q;

if exist('AVG','var')
    AVG.p_prime = zeros(Nc,Nkmax,AVG.Nt);
    AVG.p_prime(:,1,:) = 0.5*AVG.rho_prime(:,1,:).*grav.*(dz(1)+AVG.eta); % including the free surface in the pressure
    %    here's a p_prime calculation using cut cell dz values at the bottom
    for k = 2:Nkmax
        AVG.p_prime(i,k,:) = AVG.p_prime(i,k-1,:)+0.5*AVG.rho_prime(i,k-1,:).*grav.*dZ_mask(i,k-1)+...
                           0.5*AVG.rho_prime(i,k,:).*grav.*dZ_mask(i,k);
    end
    AVG.p_prime = AVG.p_prime +0*AVG.u;
end
...
%     AVG.p_hyd = AVG.p0 + AVG.p_b + AVG.p_prime;
    % clear out any pressure in non-computational cells
    AVG.p0(AVG.empties==1) = nan;
    AVG.p_b(AVG.empties==1) = nan;
    AVG.p_prime(AVG.empties==1) = nan;
    % nonhydrostatic p_nhyd = q;
    if isfield(AVG,'q')
        AVG.p_nhyd = AVG.q.*rho0;
        AVG = rmfield(AVG,'q');
%         AVG.p = AVG.p_hyd + AVG.p_nhyd;
        AVG.p_nhyd(AVG.empties==1) = nan;
    end
end

%% save profile data
disp('computing profile data...')
[PROF] = get_profile(datadir,rho0,gamma);
...
if exist('AVG','var')
%     indxavg = AVG.t>(t(end)-Tavg);
    if length(AVG.t)>2
        indxAVG = AVG.t>(AVG.t(end)-Tavg);
        if sum(indxAVG)==0
...
%     v_tildeprime = v_prime-v_barprime;
    ...
    Tbar = squeeze(nanmean(AVG.T(:,:,indxAVG),3));
    Sbar = squeeze(nanmean(AVG.S(:,:,indxAVG),3));
    
    % unsteady terms for averaging
    if length(AVG.t)>2
        dubar_dt = (AVG.u(:,:,end) - AVG.u(:,:,end-1))/(AVG.t(end)-AVG.t(end-1));
        dvbar_dt = (AVG.v(:,:,end) - AVG.v(:,:,end-1))/(AVG.t(end)-AVG.t(end-1));
        detabar_dt = (AVG.eta(:,:,end) - AVG.eta(:,:,end-1))/(AVG.t(end)-AVG.t(end-1));
        dUbar_dt = (AVG.U(:,end) - AVG.U(:,end-1))/(AVG.t(end)-AVG.t(end-1));
        dVbar_dt = (AVG.V(:,end) - AVG.V(:,end-1))/(AVG.t(end)-AVG.t(end-1));     
    end
...
z_pyn = zeros(Nc, Nout_keep);
for j=1:Nout_keep
    disp(['interp z_pyn: ' num2str(j) ' of ' num2str(Nout_keep)])
    parfor i=1:Nc
        temp = squeeze(rho(i,:,j))-rho0;
        indx = ~isnan(temp) & ~isinf(temp); % make sure no nan or inf
        temp = temp(indx);
        tempz = z(indx);  
        % reorder so can interp
        [temp,ia,~]=unique(temp);
        tempz = tempz(ia);
...
%     grav=9.81;
    ...
    % clean up and save save AVG, PROF as separate files
    if exist('AVG','var')
        disp('saving AVG file...')
        save([savedir 'SUNTANS_results_average.mat'],'-v7.3','-struct','AVG')
    end
    if exist('PROF','var')
        disp('saving PROF file...')
        save([savedir 'SUNTANS_results_prof.mat'],'-v7.3','-struct','PROF')
    end
    
    disp('saving MAT file')
    clear ans arraysize bot c cols data F i j k is ntout precision step tend teval ...
        Quad BATH DRAG Ek0_prime Ek_prime Uface empties p0 tstemp AVG PROF
