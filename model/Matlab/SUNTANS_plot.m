...
AVG = load('SUNTANS_results_average');
PROF = load('SUNTANS_results_prof');
...
for i=1:length(PROF.dataNames) 
   loc=i; 
  xlims = [min(PROF.mtime) max(PROF.mtime)];
  ylims = [ max([-PROF.d0(loc) -1000]) 0];
  
  
  figure(153324);
  subplot(6,1,1)
  plot(PROF.mtime,PROF.etaplot{i})
  xlim(xlims)
  ...
  subplot(6,1,2)  
%   pcolorjw(PROF.mtime,PROF.z{loc},...
%       PROF.uplot{loc});
  pcolor(PROF.mtime,PROF.z{loc},...
      PROF.uplot{loc});
  ylim(ylims);
  xlim(xlims)
  datetick('x','keeplimits')
...
%       PROF.p_dynamic{loc});
  ...
  subplot(6,1,5)  
  contourf(PROF.mtime,PROF.z{loc},...
      PROF.Tplot{loc},10);
  ylim(ylims);
  xlim(xlims)
  datetick('x','keeplimits')
...
Fxpbar_plot = reshape(Fx_primebar,GRID.Nx,GRID.Ny);
...
pcolor(xplot,t/3600/12.42,Fx0_plot')
cb=colorbar;
...
pcolor(xplot,t/3600/12.42,Fxp_plot')
cb=colorbar;
set(cb,'location','east')
caxis(1*c*[-1 1])
ylabel(cb,'Fx''')
