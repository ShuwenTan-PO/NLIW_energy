function [] = SUNTANS_boundary_conditions_soliton_short(n_soliton,datadir,...
...
% by S.Tan 2023/06/05
% 1) if ramp_on==0 (in suntans.dat), then no sponge layer initiated in the
% netcdf file, sources.c will take the sponge_distance as the sponge layer
% thickness and ONLY damp momentum within sponge layers rather than ramp to
% given velocity/momentum field
% 2) if ramp_on==1, then sponge_uf given in the netcdf file, but only at
% the incoming wave boundary (rn1_e==1), when reads into sources.c,
% incoming wave boundary will ramp up the waves while the outcoming wave
% boundary will damp out the waves
% by S.Tan 2023/12/27
% 1) only generate sponge layer for limited time (nbc can < nsteps)
% need to pair with new boundary.c
% 2) make sure wave maker also at sponge_uf(x2) by marking out n1(mark>0)=0;
% by S.Tan 2024/2/9
% fix bug: sponge_uf only have values for y<max(yv)/2, this is fixed by
% sponge_uf = sponge_u;
...
% this way the boundary points also can have sponge_uf
...
nbc = getvalue([datadir,'/suntans.dat'],'nbc');
...
ramp_on =  getvalue([datadir,'/suntans.dat'],'ramp_on');
if length(ramp_on)==0
    ramp_on = 1; % default ramp_on = 1 if not specified in suntans.dat
end
...
if ramp_on == 1 % method 1 or wave_nesting == 1 & sponge_distance>0 for method 2
    D_hat = exp(-4.0*r_e/sponge_distance);
else
    D_hat = exp(-4.0*r_e/20);
end
% D_hat = exp(-4.0*r_e/sponge_distance);
...
Nbc_end = ceil(dt*nbc/bc_dt)+2; % add extra pts on end
...
% if Ns*Nt*Nkmax>2e+9
%     if ramp_on == 1 % method 1 or wave_nesting == 1 & sponge_distance>0 for method 2
%         D_hat = exp(-4.0*r_e/(sponge_distance/2.2));
%         spongei = (D_hat >= exp(-4.1)) & (rn1_e>0);
%     else
%         D_hat = exp(-4.0*r_e/1);
%         spongei = D_hat >= exp(-4.1);    
...
if n_soliton==1 %one solitary wave bc
    ...
    if Ntype2
        ...
        % face fluxes
        for i=1:Nt
%             sponge_uf(:,:,i) = N2(spongei,:).*sponge_u(:,:,i) + N1(spongei,:).*sponge_v(:,:,i);
            sponge_uf(:,:,i) = sponge_u(:,:,i);% + sponge_v(:,:,i);
        end
...
j=1;
contourf(time_,z_r,squeeze(sponge_uf(j,:,:)));
title(strcat('sponge_uf at x=', string(xe_spongei(j)), ...
   'y=', string(ye_spongei(j))))
xlabel('t (s)')
ylabel('z (m)')
