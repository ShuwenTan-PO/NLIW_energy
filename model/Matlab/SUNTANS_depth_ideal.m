function [] = SUNTANS_depth_ideal(datadir,D0,shape,bathy_dir,x0,y0,theta,dtrend,dmin,extreme,INPUT)
% Justin Rogers
% Stanford University 2018
%
% update by S.Tan, UCI, 2023/02/17
% add topography "slope"
%
% datadir='../rundata';
% H=3000;
if nargin<9
...
Nk =  getvalue([datadir,'/suntans.dat'],'Nkmax');
...
elseif strcmp(shape,'rightslope')
    % right slope (1 waypoint)
    angle_slope = [0, INPUT.slope]; 
    way_point = nanmean(xv);
    Depth = nan(size(xv));
    Depth(find(xv<way_point)) = D0 - angle_slope(1).*xv(find(xv<way_point));
    Depth(find(xv>=way_point)) = D0 - angle_slope(2).*(xv(find(xv>=way_point))-way_point);
    % left slope

elseif strcmp(shape,'leftslope_slope')
    ...
    for ii = 1:Ny
        way_point1 = INPUT.waypoint+(max(yv)-BATH.y(1,ii))*0.2;
        BATH.z(find(BATH.x(:,ii)>=way_point1),ii) =  angle_slope(2).*(BATH.x(find(BATH.x(:,ii)>=way_point1),ii)-way_point1);
    end
    Depth = reshape(BATH.z,Nx*Ny,1);
    Depth(find(Depth<-INPUT.zmax))=-INPUT.zmax;

%     % left slope (2 waypoints)
%     angle_slope = [0, INPUT.slope]; 
%     way_point1 = INPUT.width_top/2;
% %     way_point2 = max(xv) - INPUT.width_top/2;
%     way_point2 = max(xv);
% %     way_point1 = 0;
% %     way_point2 = max(xv) - INPUT.width_top;
%     Depth = nan(size(xv));
% %     Depth = angle_slope(2).*xv + yv;
%     Depth(find(xv<way_point1)) =  angle_slope(1).*xv(find(xv<way_point1));
%     Depth(find((xv>=way_point1)&(xv<=way_point2))) =  angle_slope(2).*(xv(find((xv>=way_point1)&(xv<=way_point2)))-way_point1);
%     Depth(find(xv>way_point2)) = D0;
%     Depth(find(Depth<-INPUT.zmax))=-INPUT.zmax;

elseif strcmp(shape,'leftslope')
    % left slope (2 waypoints)
    angle_slope = [0, INPUT.slope]; 
    way_point1 = INPUT.width_top/2;
%     way_point2 = max(xv) - INPUT.width_top/2;
...
%     r = 30e3/2; % radius of base
%     x0 = 0.33*L;
%     y0 = W/2;
%     z0 = -300; % level of base
%     zmax = -10;
%     nx_slope = (zmax-z0)/slope/dx;
    h = INPUT.slope*INPUT.r; % height of cone
    c = INPUT.r/h;
    s = sqrt(INPUT.r^2+h^2);
    z = -sqrt(((xv-INPUT.x0).^2 + (yv-INPUT.y0).^2)./c.^2) +INPUT.z0+h;
    z(z<INPUT.z0)=INPUT.z0;
    z(z>INPUT.zmax)=INPUT.zmax;
    Depth = -z;
    ...
    save('SUNTANS_grid.mat','BATH','-append');
...
zb = z-dz/2;
zt = z+dz/2;
dZ_cut = nan+ones(length(xv),length(z));
...
XV = reshape(xv,Nx,Ny);
YV = reshape(yv,Nx,Ny);
DX = diff(XV(:,1:end-1),[],1);
DY = diff(YV(1:end-1,:),[],2);
DEPTH = reshape(Depth,Nx,Ny);
DHDX = diff(DEPTH(:,1:end-1),[],1)./DX;
DHDY = diff(DEPTH(1:end-1,:),[],2)./DY;

DZ = reshape(dZ_bot,Nx,Ny);
DZDX = DZ(1:end-1,1:end-1)./DX;
DZDY = DZ(1:end-1,1:end-1)./DY;
%% save to file
...
%%
save('SUNTANS_grid.mat','Depth','xv','yv','rstretch','Nk','z','dz','dZ_bot','DX','DY','DZ','DHDX','DHDY','DZDX','DZDY','-append');
...
subplot(3,1,1)
zplot = DX;
clim = reshape(zplot,[],1);
clim = [min(clim), max(clim)];
pcolor(XV(1:end-1,1:end-1),YV(1:end-1,1:end-1),zplot)
colorbar
hold on
contour(XV,YV,DEPTH,5,'-k')
% caxis(clim)
ylabel('y')
axis equal
text(0.5,1.1,'dx (m)','units','normalized')
...
xlabel('x')
axis equal
text(0.5,1.1,'dy (m)','units','normalized')
...
ylabel('y')
axis equal
text(0.5,1.1,'dz bot (m)','units','normalized')

print -djpeg -r300 figure_dx
close
%% plot slopes

subplot(3,1,1)
zplot = sqrt(DHDX.^2 + DHDY.^2);
clim = reshape(zplot,[],1);
clim = [min(clim), max(clim)];
pcolor(XV(1:end-1,1:end-1),YV(1:end-1,1:end-1),zplot)
colorbar
...
ylabel('slope $|dh/dx|$')
axis equal
text(0.3,1.1,'dz bot','units','normalized')

subplot(3,1,2)
zplot = sqrt(DZDX.^2 + DZDY.^2);
clim = reshape(zplot,[],1);
clim = [min(clim), max(clim)];
pcolor(XV(1:end-1,1:end-1),YV(1:end-1,1:end-1),zplot)
colorbar
...
xlabel('x')
axis equal
text(0.3,1.1,'grid slope bot $|dz/dx|$','units','normalized')

subplot(3,1,3)
zplot = sqrt(DHDX.^2 + DHDY.^2)./sqrt(DZDX.^2 + DZDY.^2);
clim = reshape(zplot,[],1);
clim = [min(clim), max(clim)];
pcolor(XV(1:end-1,1:end-1),YV(1:end-1,1:end-1),zplot)
colorbar
hold on
contour(XV,YV,DEPTH,2,'-k')
% caxis(clim)
ylabel('y')
xlabel('x')
axis equal
text(0.3,1.1,'$|dhdx|/|dzdx|$, $>1$ uneven slope','units','normalized')

print -djpeg -r300 figure_slope
close

%% plot slopes vs depth

subplot(2,2,1)
xplot = DEPTH(1:end-1,1:end-1);
xplot = reshape(xplot,[],1);
yplot = DX;
yplot = reshape(yplot,[],1);
plot(xplot,yplot,'.r')
xpp = max([max(xplot) max(yplot)]);
hold on
yplot = DY;
yplot = reshape(yplot,[],1);
plot(xplot,yplot,'.b')
...
xplot = DEPTH(1:end-1,1:end-1);
xplot = reshape(xplot,[],1);
yplot = sqrt(DZDX.^2 + DZDY.^2);
yplot = reshape(yplot,[],1);
plot(xplot,yplot,'.k')
hold on
yplot = sqrt(DHDX.^2 + DHDY.^2);
yplot = reshape(yplot,[],1);
plot(xplot,yplot,'.b')
xlabel('h (m)')
ylabel('')
...
xplot = DEPTH(1:end-1,1:end-1);
xplot = reshape(xplot,[],1);
yplot = sqrt(DHDX.^2 + DHDY.^2)./sqrt(DZDX.^2 + DZDY.^2);
yplot = reshape(yplot,[],1);
plot(xplot,yplot,'.')
hold on
plot(xplot,1+0*xplot,'-r')
xlabel('h (m)')
ylabel('$dhdx/dzdx$')
text(0.02, 0.9,'choppy derivatives','units','normalized')
text(0.02, 0.02,'smooth derivatives','units','normalized')
axis square

print -djpeg -r300 figure_slope_depth
close
%%
...
%     caxis([min(-Depth) max(-Depth)])
    ...
    %%
if strcmp(shape,'NCOM')
        load('./SUNTANS_grid_quadgrid.mat','xe','ye','mark')
        figure(23423)
        if isfield(BATH,'NC')
            
        
        pcolor(BATH.NC.X/1000,BATH.NC.Y/1000,-BATH.NC.Depth)
        cb=colorbar;
        ylabel(cb,'z (m)')
