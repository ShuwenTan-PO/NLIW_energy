...
void AllocateEnergyVariables(gridT *grid, energyT **energy, propT *prop){
  ...
  *energy = (energyT *)SunMalloc(sizeof(energyT),"AllocateEnergyVariables");
  ...
  (*energy)->p_hyd = (REAL **)SunMalloc(grid->Nc*sizeof(REAL *),"AllocateEnergyVariables");
  ...
  for(i=0;i<grid->Nc;i++) {
    ...
    (*energy)->p_hyd[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateEnergyVariables");
  ...
  for(i=0;i<grid->Nc;i++) {
    ...
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      ...
      if(k==grid->ctop[i]){
        ...
        energy->p_hyd[i][k]=energy->p0[i][k]+energy->p_b[i][k]+energy->p_prime[i][k];
      } else {
...
/*
 * Function: Conversion_z
 * Usage: Conversion_z(phys->C1_,phys->C2_,phys->W,grid,phys,prop,comm,myproc);
 * -----------------------------------------------------------------------------------------------
 * Computes the conversion term C following (17) in Kang and Fringer (2012) but as a function of z
 * Note that W and dqdz are on surface and bottom of each cell, C is calculated at center of cell
 */
void Conversion_z(REAL **C1_, REAL **C2_, gridT *grid, energyT *energy, physT *phys, propT *prop, MPI_Comm comm) {
  int i, k;
  ...
  for(i=0;i<grid->Nc;i++) {
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      C1_[i][k] = 0.5*RHO0*energy->rho_prime[i][k]*(energy->W[i][k]+energy->W[i][k+1])*prop->grav; 
      C2_[i][k] = -0.5*RHO0*(energy->dqdz[i][k]*energy->W[i][k]+energy->dqdz[i][k+1]*energy->W[i][k+1]); 
    }
  }
...
void EnergyFlux(REAL *Fx_0, REAL *Fy_0, REAL *Fx_prime, REAL *Fy_prime, gridT *grid, energyT *energy, physT *phys, propT *prop, MPI_Comm comm) {
  ...
  for(i=0;i<grid->Nc;i++) {
    ...
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      ...
      Fx_prime_2+=energy->uc_prime[i][k]*energy->Ek0_prime[i][k]*grid->dzz[i][k]; 
      ...
      Fy_prime_2+=energy->vc_prime[i][k]*energy->Ek0_prime[i][k]*grid->dzz[i][k]; 
 ...
 * ---------------------------------
 * compute pressure and store it
 */
void OutputPressure(REAL **p_prime, REAL **p_hyd, energyT *energy, gridT *grid, physT *phys, propT *prop, MPI_Comm comm){
  int i, k;
  ...
  for(i=0;i<grid->Nc;i++) {
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      p_prime[i][k] = energy->p_prime[i][k];
      p_hyd[i][k] = energy->p_hyd[i][k];
    }
  }
