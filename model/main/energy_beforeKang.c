...
static void PotentialEnergy(energyT *energy, gridT *grid, physT *phys, propT *prop) {
  int i, j, k, min_idx;
  REAL *z_smalltick, *T_smalltick, *S_smalltick, *p_smalltick, *rho_smalltick; // note small ticks are for integrating arc-triangle area, spacing is defined by Nkmax: usually 50-100 
  REAL *z_i, *z_p, *z_star, *rho_perturb, temp; 

  z_smalltick = (REAL *)SunMalloc((grid->Nkmax)*sizeof(REAL),"ComputeZ");
  T_smalltick = (REAL *)SunMalloc((grid->Nkmax)*sizeof(REAL),"ComputeZ");
  S_smalltick = (REAL *)SunMalloc((grid->Nkmax)*sizeof(REAL),"ComputeZ");
  p_smalltick = (REAL *)SunMalloc((grid->Nkmax)*sizeof(REAL),"ComputeZ");
  rho_smalltick = (REAL *)SunMalloc((grid->Nkmax)*sizeof(REAL),"ComputeZ");
  ...
  for(i=0;i<grid->Nc;i++) {
    ...
    // z for initial rho profile, not follow free surface and bottom
    z_i[0] = -grid->dz[0]*0.5;
    ...
    // compute perturbed rho = rho_b + rho_prime
    for(k=0;k<grid->Nk[i];k++){
    ...
    // find z_star, depth for perturbed density on initial density profile
    interp1(energy->rho_initial[i], z_i, grid->Nkmax, rho_perturb, z_star, grid->Nk[i]);
    for(k=0;k<grid->Nk[i];k++){
    ...
    // integrate arc-triangle (APE2)
    // 1. set z_smalltick for between z_star and z_p, 
    // 2. compute rho_smalltick based on initial conditions, 
    // 3. integrate rho_smalltick - rho_perturb over z_smalltick
    for(k=grid->ctop[i];k<grid->Nk[i];k++){
      if(!IsNan(z_star[k])){
        temp = (z_p[k]-z_star[k])/(grid->Nkmax-1);
        for(j=0;j<grid->Nkmax;j++){
          z_smalltick[j]=z_p[k]-temp*j;
          T_smalltick[j]=ReturnTemperature(grid->xv[i],grid->yv[i],z_smalltick[j],grid->dv[i]);
          S_smalltick[j]=ReturnSalinity(grid->xv[i],grid->yv[i],z_smalltick[j],prop);
          p_smalltick[j]=-RHO0*prop->grav*z_smalltick[j];
          rho_smalltick[j]=StateEquation(prop,S_smalltick[j],T_smalltick[j],p_smalltick[j]);
        }
        // integration arc-triangle
        for(j=1;j<grid->Nkmax;j++){
          energy->Ep_prime[i][k]+=RHO0*prop->grav*(rho_perturb[k]-(rho_smalltick[j-1]+rho_smalltick[j])/2)*(z_smalltick[j-1]-z_smalltick[j]);
        }     
      }
    }
    // // integration triangle (APE3)
    // for(k=grid->ctop[i];k<grid->Nk[i];k++){
    //   energy->Ep_prime[i][k]=RHO0*prop->grav*(rho_perturb[k]-energy->rho_initial[i][k])*(z_p[k]-z_star[k])/2;
    // }
  }     
