...
static void OpenDataFiles(int myproc) {
  ...
  if(strlen(ProfileVariables)>0) {
    ...
    if(ContainsCharacter(ProfileVariables,'o')) {
      MPI_GetFile(filename,DATAFILE,"EnergyFile","OpenDataFiles",myproc);
      if(myproc==0) {
        jstr = sprintf(str,"%s",filename);
        jstr += sprintf(str+jstr,"%s","a.prof");
        // sprintf(str,"%sa.prof",filename);
        EnergyProfFID = fopen(str,"w");
      }
    }  
...
static void WriteAllProfileData(gridT *grid, physT *phys, propT *prop, MPI_Comm comm, int numprocs, int myproc) {
  ...
  if(ContainsCharacter(ProfileVariables,'o')) 
    Write3DData(phys->C1_,grid->Nk,NkmaxProfs,merge_tmp,merge_tmp2,EnergyProfFID,comm,numprocs,myproc);
