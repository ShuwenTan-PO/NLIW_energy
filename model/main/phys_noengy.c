...
void AllocatePhysicalVariables(gridT *grid, physT **phys, propT *prop)
{
  ...
  *phys = (physT *)SunMalloc(sizeof(physT),"AllocatePhysicalVariables");
  ...
  for(i=0;i<Nc;i++) {
    ...
    if(prop->computeEnergy){
      (*phys)->W[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocatePhysicalVariables");
    }
  }
...
void Solve(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  for(n=prop->nstart+1;n<=prop->nsteps+prop->nstart;n++) {
    ...
    if(prop->computeEnergy){
      BarotropicW(phys->W,grid,phys,comm);
      Conversion(phys->C1,phys->C2,phys->W,grid,phys,prop,comm,myproc);
      EnergyFlux(phys->Fx_0,phys->Fy_0,phys->Fx_prime,phys->Fy_prime,grid,phys,prop,comm,myproc);
      if(prop->n==1){
