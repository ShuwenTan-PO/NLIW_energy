  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    for(k=grid->etop[j];k<grid->Nke[j];k++) {
      ...
      if(prop->wave_nesting==1){//idealized velocity
        ...
        phys->boundary_u[jind][k]=(u_LS_bar + U_LS_tilde + u_LS_tilde_prime)*tr_hat;
        phys->boundary_v[jind][k]=(v_LS_bar + V_LS_tilde + v_LS_tilde_prime)*tr_hat;
      ...
      // if (xe>498000. && k==0) printf("boundary z= %f, xe=%f, fz=%f, rtime=%f, boundary_u = %f\n",z,xe,fz,prop->rtime,phys->boundary_u[jind][k]);
     // printf("bound->boundary_u[k][bound->ind2[ii]]=%f\n",bound->boundary_u[k][bound->ind2[ii]]);
...
//    if (myproc==0) printf("t1: %f, %t2: %f, tnow:%f, ,mu %f, r2: %f, r1: %f\n",bound->time[bound->t0],bound->time[bound->t1], prop->nctime, mu, r2, r1);
    ...
    if(prop->wave_nesting==2){ // interpolate sponge_u, may want to implement some kind of threshold, so not interpolating every time step
      ...
      if(prop->lowfreq_nudging==1 && bound->LpInterpCounter > prop->ntaverage-1){ // interpolate low pass varibles
        //printf("quadinterp low freq myproc=%d, lpcounter=%d\n",myproc,bound->LpInterpCounter );
        for (j=0;j<bound->Ne;j++){
          for (k=0;k<bound->Nk;k++){
            //Quadratic temporal interpolation
            bound->lowfreq_uf[k][j] = QuadInterp(prop->nctime,bound->time[t0],bound->time[t1],bound->time[t2],bound->lowfreq_uf_t[0][k][j],bound->lowfreq_uf_t[1][k][j],bound->lowfreq_uf_t[2][k][j] );
...
void AllocateBoundaryData(propT *prop, gridT *grid, boundT **bound, int myproc, MPI_Comm comm){
  ...
  Ne = returndimlenBC(prop->netcdfBdyFileID,"Ne");
    ...
    */
    ...
    (*bound)->Nk=Nk;
    (*bound)->Ne=Ne;
    (*bound)->SpgInterpCounter=10; // counter for sponge interpolation
    (*bound)->LpInterpCounter=10; // counter for low freq interpolation
  ...
  for(k=0;k<Nk;k++){
    ...
    if(prop->wave_nesting==2){
      if(myproc==0){
        printf("initializing sponge variables, wave_nesting=%d, Ne=%d, Nk=%d, NT=%d\n",prop->wave_nesting,Ne,Nk,NT );
      }
      ...
      for(k=0;k<Nk;k++){
        (*bound)->sponge_uf[k] = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocateBoundaryData");
        ...
        if(prop->lowfreq_nudging)
        (*bound)->lowfreq_uf[k] = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocateBoundaryData");
      ...
      for(n=0;n<NT;n++){
        ...
        for(k=0;k<Nk;k++){
          (*bound)->sponge_uf_t[n][k] = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocateBoundaryData");
          ...
          if(prop->lowfreq_nudging)
          (*bound)->lowfreq_uf_t[n][k] = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocateBoundaryData");
      ...
      for(n=0;n<1;n++){
        (*bound)->ncscratch1[n] = (REAL **)SunMalloc(Nk*sizeof(REAL *),"AllocateBoundaryData");
        // (*bound)->ncscratch2[n] = (REAL **)SunMalloc(Nk*sizeof(REAL *),"AllocateBoundaryData");
        for(k=0;k<Nk;k++){
         (*bound)->ncscratch1[n][k] = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocateBoundaryData");
    ...
    (*bound)->indSponge = (int *)SunMalloc(Ne*sizeof(int),"AllocateBoundaryData");
    ...
    if(prop->wave_nesting==2){
      for(j=0;j<Ne;j++){
        for(k=0;k<Nk;k++){
          ...
          // (*bound)->sponge_u[k][j]=0.00;
          // (*bound)->sponge_v[k][j]=0.00;
          if(prop->lowfreq_nudging)
          (*bound)->lowfreq_uf[k][j]=0.00;
          for(n=0;n<NT;n++){
            (*bound)->sponge_uf_t[n][k][j]=0.00;
            // (*bound)->sponge_u_t[n][k][j]=0.00;
            // (*bound)->sponge_v_t[n][k][j]=0.00;
            if(prop->lowfreq_nudging)
