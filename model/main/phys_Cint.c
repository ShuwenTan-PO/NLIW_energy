...
static void StoreEnergyVariables(gridT *grid, physT *phys);
static void AverageEnergyVariables(gridT *grid, physT *phys, propT *prop);
static void StoreEnergyVariablesDecompose(gridT *grid, physT *phys);
...
void Solve(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  for(n=prop->nstart+1;n<=prop->nsteps+prop->nstart;n++) {
    ...
    if(prop->computeEnergy){
      ...
      if(prop->energyfluxCombine){
        if(prop->n==1){
          StoreEnergyVariables(grid,phys);
        }
        AverageEnergyVariables(grid,phys,prop);
        StoreEnergyVariables(grid,phys);
      }else{
        if(prop->n==1){
          StoreEnergyVariablesDecompose(grid,phys);
        }
